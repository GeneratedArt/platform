function c(e){return e.replace(/[&<>"']/g,t=>{switch(t){case"&":return"&amp;";case"<":return"&lt;";case">":return"&gt;";case'"':return"&quot;";default:return"&#39;"}})}async function b(e){let t=await fetch(e,{credentials:"include"});return t.ok?await t.json():{__status:t.status}}function f(e){return typeof e=="object"&&e!==null&&"__status"in e}function w(e){switch(e){case"minted":return"ga-badge ga-badge-minted";case"archived":return"ga-badge ga-badge-archived";case"draft":return"ga-badge ga-badge-draft";default:return"ga-badge ga-badge-published"}}var y={draft:"Draft",published:"Published",minted:"Minted",archived:"Archived"};async function $(e,t,i){let n=document.getElementById("ga-project-tmpl");if(!n)return;let a=n.content.cloneNode(!0),r=a.querySelector(".ga-project-detail");r.querySelector(".ga-project-title").textContent=t.title,r.querySelector(".ga-project-engine").textContent=t.engine,r.querySelector(".ga-project-license").textContent=t.license;let d=r.querySelector(".ga-project-status");d.innerHTML=`<span class="${w(t.status)}">${c(y[t.status]||t.status)}</span>`;let u=r.querySelector(".ga-project-description");u.textContent=t.description||"No description yet.";let l=r.querySelector(".ga-project-repo");t.repo_url?(l.href=t.repo_url,l.textContent=(t.repo_full||t.repo_url)+" \u2197"):l.parentElement.innerHTML='<span class="text-muted">Repo not linked.</span>';let g=r.querySelector(".ga-project-cover");t.cover_url&&(g.innerHTML=`<img src="${c(t.cover_url)}" alt="${c(t.title)}" />`);let s=r.querySelector(".ga-project-updated"),o=new Date(t.updated_at*1e3);s.textContent=`Updated ${o.toLocaleDateString()}`;let p=r.querySelector(".ga-project-author");i?(p.href=`/@${i.handle}/`,p.textContent=`@${i.handle}`):(p.removeAttribute("href"),p.textContent=`artist #${t.owner_id}`),e.rootEl.innerHTML="",e.rootEl.appendChild(a),document.title=`${t.title} \u2014 GeneratedArt`}function h(e,t){e.rootEl.innerHTML=`
    <div class="text-center py-10">
      <h1 class="h3 mb-3">Project not found</h1>
      <p class="text-muted small mb-4">${c(t)}</p>
  ...
  `,i.appendChild(n);let a=n.querySelector(".ga-freeze-list"),r=n.querySelector(".ga-freeze-actions"),d=n.querySelector(".ga-freeze-status"),u=n.querySelector("[data-action='freeze']"),l=!1;try{let s=await b(`${e.apiBase}/v1/me`);f(s)||(l=s.user.id===t.owner_id)}catch{l=!1}l&&r.classList.remove("d-none");async function g(){let s=await b(`${e.apiBase}/v1/projects/${t.id}/frozen`);if(f(s)){a.innerHTML=`<p class="text-muted">Couldn't load frozen versions (${s.__status}).</p>`;return}if(s.versions.length===0){a.innerHTML=`<p class="text-muted">No frozen versions yet.${l?" Click <em>Freeze current commit</em> to create one.":""}</p>`;return}a.innerHTML=s.versions.map(o=>z(o,l)).join(""),a.querySelectorAll("[data-activate]").forEach(o=>{o.addEventListener("click",async()=>{o.disabled=!0,o.textContent="Activating\u2026";let p=o.getAttribute("data-activate"),m=await fetch(`${e.apiBase}/v1/projects/${t.id}/frozen/${p}/activate`,{method:"POST",credentials:"include"});if(!m.ok){let v=await m.json().catch(()=>({}));d.textContent=`Activate failed: ${v.error||m.status}`}await g()})})}u?.addEventListener("click",async()=>{u.disabled=!0,d.textContent="Building bundle + pinning\u2026";let s=await fetch(`${e.apiBase}/v1/projects/${t.id}/freeze`,{method:"POST",credentials:"include",headers:{"Content-Type":"application/json"},body:JSON.stringify({commit:"latest"})});if(u.disabled=!1,!s.ok){let o=await s.json().catch(()=>({}));d.textContent=`Freeze failed: ${o.error||s.status}`;return}d.textContent="Frozen. Activate it below to make it the live version.",await g()}),await g()}function z(e,t){let i=(e.bytes/1024).toFixed(1),n=new Date(e.created_at*1e3).toLocaleString(),a=[e.pinned_w3s?"\u2713 web3.storage":"\u2717 web3.storage",e.pinned_pinata?"\u2713 Pinata":"\u2717 Pinata"].join(" \xB7 "),r=e.pinning_partial?'<span class="ga-badge ga-badge-archived ms-2">Partial pin</span>':"",d=e.is_active?'<span class="ga-badge ga-badge-minted ms-2">Active</span>':"",u=t&&!e.is_active&&(e.pinned_w3s||e.pinned_pinata)?`<button type="button" class="btn btn-sm btn-outline-primary rounded-0 ms-2" data-activate="${e.id}">Activate</button>`:"";return`
    <div class="ga-freeze-row" style="border:1px solid var(--ga-rule); padding:12px; margin-bottom:8px;">
      <div class="d-flex justify-content-between align-items-center mb-1">
        <code style="font-size:12px;">${c(e.cid)}</code>
        <span>${d}${r}${u}</span>
      </div>
      <div class="text-muted" style="font-size:12px; line-height:1.5;">
        <div>commit <code>${c(e.commit_sha.slice(0,12))}</code> \xB7 sha256 <code>${c(e.bundle_hash.slice(0,16))}\u2026</code> \xB7 ${i} KB</div>
        <div>${a} \xB7 ${c(n)}</div>
        <div>
          <a href="${c(e.gateways.w3s)}" target="_blank" rel="noopener">w3s.link \u2197</a>
          \xB7
          <a href="${c(e.gateways.pinata)}" target="_blank" rel="noopener">pinata \u2197</a>
        </div>
      </div>
    </div>
