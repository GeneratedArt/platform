var p={draft:"Draft",published:"Published",minted:"Minted",archived:"Archived"};function g(e){return`<span class="${`ga-badge ga-badge-${e}`}">${p[e]||e}</span>`}function o(e){return e.replace(/[&<>"']/g,t=>{switch(t){case"&":return"&amp;";case"<":return"&lt;";case">":return"&gt;";case'"':return"&quot;";default:return"&#39;"}})}function v(e){let t=e.status==="archived",r=e.repo_url?`<a href="${o(e.repo_url)}" target="_blank" rel="noopener" class="ga-card-link">View repo \u2192</a>`:"",a=t?"":`<button class="btn btn-link p-0 text-danger ga-archive-btn" data-project-id="${e.id}">Archive</button>`;return`
  ...
  `}async function c(e,t){let r=await fetch(e,{credentials:"include",...t});if(!r.ok){let a="";try{a=JSON.stringify(await r.json())}catch{}throw new Error(`${r.status}: ${a||r.statusText}`)}return await r.json()}async function d(e){let t=e.rootEl.querySelector("#ga-project-list");if(t){t.innerHTML='<div class="text-center p-4 text-muted">Loading\u2026</div>';try{let{projects:r}=await c(`${e.apiBase}/v1/projects/mine`);r.length?t.innerHTML=r.map(v).join(""):t.innerHTML=b()}catch(r){t.innerHTML=`<div class="alert alert-danger">Failed to load projects: ${o(String(r))}</div>`}}}async function f(e,t){return c(`${e.apiBase}/v1/projects`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)})}async function y(e,t){return c(`${e.apiBase}/v1/projects/${t}/archive`,{method:"POST"})}function j(e){e.rootEl.addEventListener("click",async r=>{let a=r.target;if(a.matches(".ga-archive-btn")){let n=parseInt(a.getAttribute("data-project-id")||"",10);if(!n||!confirm("Archive this project? The GitHub repo will be archived but its history is preserved."))return;try{await y(e,n),await d(e)}catch(s){alert("Archive failed: "+s)}}}),e.rootEl.querySelector("#ga-new-project-form")?.addEventListener("submit",async r=>{r.preventDefault();let a=e.rootEl.querySelector("#ga-np-title"),n=e.rootEl.querySelector("#ga-np-description"),s=e.rootEl.querySelector("#ga-np-engine"),u=e.rootEl.querySelector("#ga-np-license"),i=e.rootEl.querySelector("#ga-new-project-error");if(!(!a||!s||!u||!i)){i.classList.add("d-none");try{await f(e,{title:a.value,description:n?.value||null,engine:s.value,license:u.value}),a.value="",n&&(n.value="");let l=document.getElementById("ga-new-project-modal");l&&window.bootstrap?.Modal&&window.bootstrap.Modal.getOrCreateInstance(l).hide(),await d(e)}catch(l){i.textContent="Create failed: "+String(l),i.classList.remove("d-none")}}})}var m={async mount(e){let t=null;try{t=await c(`${e.apiBase}/v1/me`)}catch{t=null}if(!t){e.onUnauthenticated?.();return}e.rootEl.querySelector("#ga-dashboard-handle").textContent=t.user.handle,e.rootEl.querySelector("#ga-dashboard-address").textContent=t.user.address;let r=e.rootEl.querySelector("#ga-modal-root");r&&!r.innerHTML.trim()&&(r.innerHTML=h()),j(e),await d(e)}};window.GADashboard=m;var E=m;export{E as default};
